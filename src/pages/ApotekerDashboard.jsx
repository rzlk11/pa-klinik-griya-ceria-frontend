...
function ApotekerDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [obatRes, resepRes] = await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-green-800 mb-4">Selamat Datang di Portal Apoteker</h2>
        <p className="text-gray-600">Gunakan menu di sebelah kiri untuk mengelola stok obat dan melayani resep obat.</p>
      </div>
    </div>
  );
}
